...
def getsize(font, text: str) -> tuple[int, int]:
...
def makeimg(size: tuple[int, int]) -> ImageType:
    if settings.CAPTCHA_BACKGROUND_COLOR == "transparent":
        image: ImageType = Image.new("RGBA", size)
    else:
        image: ImageType = Image.new("RGB", size, settings.CAPTCHA_BACKGROUND_COLOR)
    return image


def captcha_image(request: HttpRequest, key, scale: int=1) -> HttpResponse:
    if scale == 2 and not settings.CAPTCHA_2X_IMAGE:
        raise Http404
    try:
        store: CaptchaStore = CaptchaStore.objects.get(hashkey=key)
    ...
    if isinstance(settings.CAPTCHA_FONT_PATH, str):
        fontpath: str = settings.CAPTCHA_FONT_PATH
    elif isinstance(settings.CAPTCHA_FONT_PATH, (list, tuple)):
        fontpath: str = random.choice(settings.CAPTCHA_FONT_PATH)
    else:
    ...
    for char in charlist:
        fgimage: ImageType = Image.new("RGB", size, settings.CAPTCHA_FOREGROUND_COLOR)
        charimage: ImageType = Image.new("L", getsize(font, " %s " % char), "#000000")
        chardraw: ImageDrawType = ImageDraw.Draw(charimage)
        chardraw.text((0, 0), " %s " % char, font=font, fill="#ffffff")

        if settings.CAPTCHA_LETTER_ROTATION:
            charimage: ImageType = charimage.rotate(
                random.randrange(*settings.CAPTCHA_LETTER_ROTATION),
                expand=False,
                resample=Image.BICUBIC,
            )

        charimage: ImageType = charimage.crop(charimage.getbbox())
        maskimage: ImageType = Image.new("L", size)
        ...
        size: tuple[int, int] = maskimage.size
        image: ImageType = Image.composite(fgimage, image, maskimage)
        xpos: int = xpos + 2 + charimage.size[0]

    if settings.CAPTCHA_IMAGE_SIZE:
        # centering captcha on the image
        tmpimg: ImageType = makeimg(size)
        tmpimg.paste(
            image,
            (
                int((size[0] - xpos) / 2),
        ...
        image: ImageType = tmpimg.crop((0, 0, size[0], size[1]))
    else:
        image: ImageType = image.crop((0, 0, xpos + 1, size[1]))
...
def captcha_audio(request: HttpRequest, key) -> HttpResponse | RangedFileResponse:
    if settings.CAPTCHA_FLITE_PATH:
        ...
        path: str = str(os.path.join(tempfile.gettempdir(), "%s.wav" % key))
        ...
        if settings.CAPTCHA_SOX_PATH:
            arbnoisepath: str = str(
                os.path.join(tempfile.gettempdir(), "%s_arbitrary.wav") % key
            )
            mergedpath: str = str(os.path.join(tempfile.gettempdir(), "%s_merged.wav") % key)
            subprocess.call(
                [
        ...
        if os.path.isfile(path):
            ...
            temporary_file: BufferedRandom = tempfile.TemporaryFile()
            ...
            response: RangedFileResponse = RangedFileResponse(
...
def captcha_refresh(request: HttpRequest) -> HttpResponse:
    ...
    new_key: str = CaptchaStore.pick()
    to_json_response: dict[str, str | None] = {
        "key": new_key,
